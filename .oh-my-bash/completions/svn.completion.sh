...
_svn() {
	...
	for opt in "${COMP_WORDS[@]}"; do
		...
		if [[ ! $cmd && $opt &&
			...
			if [[ $isHelpCmd && $cmd && $cmd != 'help' && ! $help ]]; then
				help=$cmd
				cmd='help'
			fi
			last='cmd'
		...
		if [[ $isHelpCmd && ! $help && $opt && $opt != -* ]]; then
			help=$opt
			last='help'
			continue
		fi
		...
		# PROPerty name
		if [[ $isPropCmd && ! $prop && $opt && $opt != -* ]]; then
			prop=$opt
	...
	if [[ $isPropCmd && (! $prop || $stat = 'prop') && $cur != -* ]]; then
		#
	...
	if [[ $nExpectArgs && $nargs -gt $nExpectArgs ]]; then
