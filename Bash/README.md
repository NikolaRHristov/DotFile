...
_Powerline's theme is the default one. It's not the fanciest one. It's not the
...
The default location is `~/Bash` (hidden in your home directory)
...
```shell
cp ~/Bash/templates/bashrc.osh-template ~/.bashrc
```
